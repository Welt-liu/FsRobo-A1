...
class Arm_contorl(Node):
    ...
    def convert_to_servo_angle(self,joint_name,joint_postion):
        if self.joint_[joint_name] == 0:
            ...
            self.uservo.set_servo_angle(3,radians_to_degrees(joint_postion),velocity=750)
        elif self.joint_[joint_name] == 4:
            self.uservo.set_servo_angle(4,-90+radians_to_degrees(joint_postion),velocity=750)
        elif self.joint_[joint_name] == 5:
            self.uservo.set_servo_angle(5,-radians_to_degrees(joint_postion),velocity=750)
